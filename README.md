...
A privacy-first browser extension that automatically syncs Gradescope assignments to Google Calendar with comprehensive grade tracking and calculation.

**Current Version: v2.0.1** — Available on Chrome Web Store & Firefox Add-ons

## Features

### Core Functionality
- **Automatic dashboard discovery**: Extracts all assignments from all enrolled courses
- **Comprehensive grade calculator**:
  - Automatic assignment categorization (homework, lab, exam, project, etc.)
  - Per-course grade configuration with custom weighting
  - Real-time grade tracking with submission status
  - Visual grade breakdown by category
  - "What-if" scenario planning for future grades
...
2. **Automatic extraction**: Visit any Gradescope page - assignments are automatically discovered and extracted with grade data
3. **Instant first sync**: On first authentication, all extracted assignments immediately sync to your calendar
4. **Track your grades**: View comprehensive grade breakdowns, configure course weighting, and plan future performance
5. **Stay updated automatically**: Four sync mechanisms keep your calendar current:
...
6. **Customize your events**: Set colors, reminders, and display timing in the options page
7. **Export if needed**: Generate iCal files for Outlook, Apple Calendar, or other calendar apps
8. **Cross-device access**: Calendar events sync to all your devices via Google Calendar

## Version History

### v2.0.1 (Current) - Critical Bug Fixes
- **Fixed duplicate calendar events bug**: Restored deduplication logic lost in v2.0 merge
- **Comprehensive lastSyncType tracking**: Track which sync mechanism last updated calendar
- Improved event cache fallback behavior for reliability
- Display sync type in statistics (manual, auto, first-time, smart, background poll)

### v2.0 - Grade Calculator Launch
- **Comprehensive grade calculator** with automatic categorization
- Per-course grade configuration with custom weighting
- Real-time grade tracking with submission status
- Visual grade breakdown by assignment category
- "What-if" scenario planning for future performance
- Enhanced assignment extraction with grade data

### v1.9.2 - Bug Fixes
- Fixed deduplication logic to use correct assignmentId field
- Updated assignment counting to show only upcoming assignments
- Note: These fixes were lost in v2.0 merge and restored in v2.0.1

### v1.9.1 - Firefox Compatibility
- Fixed Firefox compatibility for iCal export (message passing bug)
- Improved cross-browser message passing with hybrid callback pattern
...
- [x] **Comprehensive grade calculator with automatic categorization**
- [x] **Per-course grade configuration and weighting system**
...
- **Grade Extraction System**: Parses grade data, submission status, and assignment metadata
- **Assignment Categorizer**: ML-style automatic categorization using pattern matching and confidence scoring
- **Course Configuration Manager**: Per-course grade weighting and custom category definitions
- **Background Service Worker**: Handles OAuth authentication, calendar sync, and auto-sync scheduling
- **Event Cache**: Performance layer with 10-minute refresh cycle, handles up to 1000 events with smart deduplication
...
- **Local Storage**: Stores assignment data, grade configurations, and user preferences in browser
...
- Individual course page extraction with grade data
- Grade calculator accuracy and categorization
- Course configuration and custom weighting
- Calendar event creation with custom colors/reminders
- Event deduplication (re-sync same assignments - should not create duplicates)
...
Built for UC Berkeley students. Usable by all Gradescope users worldwide. Inspired by the need for better assignment tracking and grade management without compromising privacy.
